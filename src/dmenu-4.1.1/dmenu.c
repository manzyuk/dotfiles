...
static void drawprompt(const char *text, unsigned long col[ColLast]);
...
static unsigned int xinescreen = -1;
...
drawcursor(void) {
  char buf[1];
  XRectangle r = { dc.x, dc.y + 2, 8, dc.font.height };
  ...
  if (buf[0])
    XDrawString(dpy, dc.drawable, dc.gc, r.x, r.y + dc.font.height - 3, buf, 1);
}
...
void
drawprompt(const char *text, unsigned long col[ColLast]) {
  char buf[256];
  int i, x, y, h, len, olen;
  XRectangle r = { dc.x, dc.y, dc.w, dc.h };
...
setup(Bool topbar) {
  ...
  if(parent == RootWindow(dpy, screen) && XineramaIsActive(dpy) && (info = XineramaQueryScreens(dpy, &n))) {
    if(xinescreen >= 0 && xinescreen < n)
      i = xinescreen;
    else {
      /* if no or an invalid Xinerama screen parameter is given, find the screen that contains the pointer */
      i = 0;
      if(n > 1) {
        int di;
        unsigned int dui;
  ...
  if(prompt)
    promptw = textnw(prompt, strlen(prompt));
  text[0] = '\0';
  match(text);
  XMapRaised(dpy, win);
}
...
main(int argc, char *argv[]) {
  ...
  for(i = 1; i < argc; i++)
    if(!strcmp(argv[i], "-i")) {
      ...
      if(++i < argc) xinescreen = atoi(argv[i]);
    }
      ...
      eprint("usage: dmenu [-i] [-b] [-l <lines>] [-fn <font>] [-nb <color>] [-nf <color>]\n"
             "             [-p <prompt>] [-sb <color>] [-sf <color>] [-xs <screen>] [-v]\n");
